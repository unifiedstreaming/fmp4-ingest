...
9.	The encoder or ingest source MAY use chunked transfer encoding option of the HTTP POST command [5] for uploading as it might be difficult to predict the entire content length of the segment. This can be used for example to support use cases that require low latency.
...
18.	The fragment decode timestamps "tfdt" of fragments in the fragmentedMP4stream and the indexes base_media_decode_ time SHOULD arrive in increasing order for each of the different tracks/streams that are ingested. 
...
20. The manifest SHOULD be used to signal the language, which SHOULD also be signalled in the "mdhd" box or "elng" boxes in the init segment and/or moof headers ("mdhd")
